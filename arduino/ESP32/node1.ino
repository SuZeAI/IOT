...
void setup() {
  Serial.begin(115200);
  mySerial.begin(115200, SERIAL_8N1, RXD2, TXD2);  // Khởi tạo UART2 cho ESP32
  ...
  pinMode(A0, INPUT);  // Cảm biến độ ẩm đất
  pinMode(A1, INPUT);
  
  pinMode(maybom, OUTPUT);  // Điều khiển máy bơm
  pinMode(quat, OUTPUT);    // Điều khiển quạt
...
void automode() {
  if (soil_humi < 60) {
    digitalWrite(maybom, LOW);  // Bật máy bơm
  } else if (soil_humi > 80) {
    digitalWrite(maybom, HIGH);  // Tắt máy bơm
  }

  if (temp > 30) {
    digitalWrite(quat, LOW);  // Bật quạt
  } else {
    digitalWrite(quat, HIGH);  // Tắt quạt
  }
}
